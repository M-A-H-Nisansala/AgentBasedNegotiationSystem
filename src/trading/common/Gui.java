...
public class Gui extends JFrame {
	...
	{
		super((GuiPanel.isBuyer(agent) ? "Buyer: " : "Seller: ")
		...
		agent.scheduleStep(new IComponentStep<Void>() {
			...
			public IFuture<Void> execute(IInternalAccess ia) {
				...
				ia.subscribeToEvents(IMonitoringEvent.TERMINATION_FILTER,
						false, PublishEventLevel.COARSE)
						.addResultListener(
								new SwingIntermediateResultListener<IMonitoringEvent>(
										new IntermediateDefaultResultListener<IMonitoringEvent>() {
											public void intermediateResultAvailable(
													IMonitoringEvent result) {
												dispose();
											}
										}));
